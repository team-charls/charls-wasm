...
class JpegLSDecoder {
  ...
  constructor(module) {
    ...
    if (!this.#decoder) {
      throw new JpegLSError('Failed to create WASM JPEG-LS decoder');
    }
   ...
   * @throws {JpegLSError} If decoder recreation fails
   */
  reset() {
    if (this.#resetNeeded) {
      this.#module._charls_jpegls_decoder_destroy(this.#decoder);
   ...
   * @throws {JpegLSError} If any step of the decoding process fails
   */
  decode(sourceBuffer) {
    this.reset();
   ...
   * @param {Uint8Array} sourceBuffer - The encoded JPEG-LS data
   * @throws {JpegLSError} If buffer setup or validation fails
   */
   ...
   * @throws {JpegLSError} If header parsing fails
   */
  readHeader() {
   ...
   * @throws {JpegLSError} If size query fails
   */
  getDestinationSize() {
